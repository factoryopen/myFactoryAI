<template>
    <div class="app-container">
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button type="primary" icon="el-icon-s-ticket" size="mini" :disabled="multiple" @click="handleDeliveryDirectly"
                >转单</el-button>
            </el-col>
          <el-col :span="1.5">
              <el-button type="primary" icon="el-icon-s-promotion" size="mini" :disabled="multiple" @click="handleDeliveryQuickly"
              >发货</el-button>
          </el-col>
            <el-col :span="1.5">
        ...
        <el-dialog :title="title" class="dialogConfig" :visible.sync="open" width="500px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                ...
                <el-form-item prop="salesId" class="marginBottom0">
                  ...
                  <el-select class="inputWidth" v-model="form.salesId" filterable placeholder="请选择销售员" size="mini" >
                    <el-option
...
import { enable,deliveryDirectly,deliveryQuickly } from "@/api/ord/soQuickly";
...
/**主表外键SalesId被选择后触发事件*/
        ...
        handleDeliveryDirectly(row) {
            const ids = row.id || this.ids;
            const billNoz = row.billNo || this.captions;
            this.$modal.confirm('确定转单 便捷订单号为"' + billNoz + '"的数据项吗？').then(function() {
                 return deliveryDirectly(ids);
            }).then(response => {
                this.getList();
                this.$modal.msgSuccess(response.msg);
            });
        },
        handleDeliveryQuickly(row) {
          const ids = row.id || this.ids;
          const billNoz = row.billNo || this.captions;
          this.$modal.confirm('确定发货 便捷订单号为"' + billNoz + '"的数据项吗？').then(function() {
