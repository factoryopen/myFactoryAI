<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="品类编号" align="left" prop="categoryNo" width="250"/>
    ...
    <el-dialog :title="title" class="dialogConfig" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
        ...
        <el-form-item prop="materialType" class="marginBottom0">
          <span slot="label">
                 <span class="fontSize12"><strong>品类类型</strong></span>
          </span>
          <el-select class="inputWidth" v-model="form.materialType" filterable placeholder="请选择品类类型" size="mini">
            <el-option
              v-for="dict in dict.type.eng_material_producttype"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item prop="lineId" class="marginBottom0">
          ...
          <el-select class="inputWidth" v-model="form.lineId" filterable placeholder="请输入产线ID" size="mini">
            <el-option
...
import { listProductCategory, getCategory, delCategory, addCategory, updateCategory } from "@/api/eng/category";
import { listLine} from "@/api/res/line";

export default {
  name: "Category",
  dicts: ['eng_material_producttype', 'eng_material_purchasingtype', 'eng_material_consumingtype', 'object_status_enable'],
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除品类名称为"' + categoryNamez + '"的数据项？').then(function() {
