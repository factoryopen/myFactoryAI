<template>
  <div class="app-container">
    ...
    <el-table ref="masterTable" v-loading="loading" border
      ...
      <el-table-column label="良品数" align="right" prop="quantityQualified"  width="80"/>
      ...
      <el-table-column label="采购类型" align="center" prop="purchaseType" width="75">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.eng_officepurchasing_type" :value="scope.row.purchaseType"/>
        </template>
      </el-table-column>
      <el-table-column label="消耗类型" align="center" prop="consumeType" width="85">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.eng_material_consumingtype" :value="scope.row.consumeType"/>
        </template>
      </el-table-column>
      <el-table-column label="生产损耗(%)" align="right" prop="rateLoss"  width="80"/>
      <el-table-column label="采购周期(天）" align="right" prop="daysCycletime"  width="80"/>
      <el-table-column label="采购提前期(天）" align="right" prop="daysLeadtime"  width="90"/>
      <el-table-column label="采购后置期(天）" align="right" prop="daysPosttime"  width="90"/>
    ...
    <el-dialog :title="title" class="dialogConfig" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
        ...
        <el-form-item prop="compoundId" class="marginBottom0">
          ...
          <treeselect v-model="form.compoundId"
                      class="inputWidth"
        ...
        <el-form-item  prop="materialSpec" class="marginBottom0">
          ...
          <el-input class="inputWidth" v-model="form.materialSpec" placeholder="请输入规格型号" @change="onMaterialSpecChanged" size="mini" />
        </el-form-item>
        ...
        <el-form-item  prop="purchaseType" class="marginBottom0">
                    <span slot="label">
                        <span class="fontSize12"><strong>采购类型</strong></span>
                    </span>
          <el-select class="inputWidth" v-model="form.purchaseType" placeholder="请选择采购类型" size="mini" >
            <el-option
              v-for="dict in dict.type.eng_officepurchasing_type"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item  prop="daysCycletime" class="marginBottom0">
          ...
          <el-input class="inputWidth" v-model="form.daysCycletime" placeholder="请输入采购周期" size="mini" />
        </el-form-item>
        <el-form-item  prop="daysLeadtime" class="marginBottom0">
                    <span slot="label">
                        <span class="fontSize12"><strong>采购提前期(天)</strong></span>
                    </span>
          <el-input class="inputWidth" v-model="form.daysLeadtime" placeholder="请输入提采购提前期" size="mini" />
        </el-form-item>
        <el-form-item  prop="daysPosttime" class="marginBottom0">
                    <span slot="label">
                        <span class="fontSize12"><strong>采购后置期(天)</strong></span>
                    </span>
          <el-input class="inputWidth" v-model="form.daysPosttime" placeholder="请输入采购后置期" size="mini" />
        </el-form-item>
...
<script>
import { listUtility, getMaterial, delMaterial, addMaterial, updateMaterial } from "@/api/eng/material";
...
import { listUtilityCategory} from "@/api/eng/category";
...
export default {
  name: "Material",
  dicts: ['eng_material_basictype', 'eng_officepurchasing_type', 'eng_material_consumingtype', 'resource_status_basic', 'sys_normal_disable', 'bas_parameter_type'],
  ...
  methods: {
    ...
    getMasterList() {
      this.loading = true;
      listUtility(this.queryParams).then(response => {
    ...
    /** 查询物料分类管理下拉树结构 */
    getTreeselect() {
      listUtilityCategory().then(response => {
