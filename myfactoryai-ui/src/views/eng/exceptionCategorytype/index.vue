    ...
    <div class="app-container">
                ...
                <el-row :gutter="10" class="mb8">
                    <el-col :span="1.5">
                        <el-button
                            ...
                            plain
                            icon="el-icon-plus"
                            @click="handleAddEngExceptionReasontype"
                        >新增</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            @click="handleUpdateEngExceptionReasontype"
                        >修改</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            @click="handleDeleteEngExceptionReasontype"
                        >删除</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            icon="el-icon-download"
                            :disabled="multipleExceptionReasontype"
                            @click="handleExportEngExceptionReasontype"
                        >导出</el-button>
                    </el-col>
                </el-row>
                    ...
                    <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width" width="126">
                       <template slot-scope="scope">
                           <el-button
                               ...
                               type="text"
                               icon="el-icon-edit"
                               @click="handleUpdateEngExceptionReasontype(scope.row)"
                           >修改</el-button>
                           <el-button
                               size="mini"
                               type="text"
                               icon="el-icon-delete"
                               @click="handleDeleteEngExceptionReasontype(scope.row)"
                           >删除</el-button>
                       </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
            <el-tab-pane label="工艺异常处理类型">
                <el-row :gutter="10" class="mb8">
                    <el-col :span="1.5">
                        <el-button
                            size="mini"
                            ...
                            plain
                            icon="el-icon-plus"
                            @click="handleAddEngExceptionSolutiontype"
                        >新增</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            plain
                            icon="el-icon-edit"
                            :disabled="singleExceptionSolutiontype"
                            @click="handleUpdateEngExceptionSolutiontype"
                        >修改</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            plain
                            icon="el-icon-delete"
                            :disabled="multipleExceptionSolutiontype"
                            @click="handleDeleteEngExceptionSolutiontype"
                        >删除</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
                            ...
                            icon="el-icon-download"
                            :disabled="multipleExceptionSolutiontype"
                            @click="handleExportEngExceptionSolutiontype"
                        >导出</el-button>
                    </el-col>
                </el-row>
                    ...
                    <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width" width="126">
                       <template slot-scope="scope">
                           <el-button
                               ...
                               type="text"
                               icon="el-icon-edit"
                               @click="handleUpdateEngExceptionSolutiontype(scope.row)"
                           >修改</el-button>
                           <el-button
                               size="mini"
                               type="text"
                               icon="el-icon-delete"
                               @click="handleDeleteEngExceptionSolutiontype(scope.row)"
                           >删除</el-button>
                       </template>
                    </el-table-column>
                </el-table>
        ...
        <el-dialog :title="title" class="dialogConfig" :visible.sync="openExceptionReasontype" width="500px" append-to-body>
            <el-form ref="formExceptionReasontype" :model="formExceptionReasontype" :rules="rulesExceptionReasontype" label-width="140px" :inline-message="true">
                    ...
                    <el-input class="inputWidth" v-model="formExceptionReasontype.categoryId" placeholder="请输入异常分类ID" size="mini" disabled="true"/>
                  </el-form-item>
                   ...
                   <el-input class="inputWidth" v-model="formExceptionReasontype.status" v-show="false"/>
                   <el-form-item prop="remark" class="marginBottom0">
                      <span slot="label">
                          <span class="fontSize12"><strong>备注</strong></span>
                      </span>
        ...
        <!-- 创建slaver.openExceptionSolutiontype对话框 -->
        <el-dialog :title="title" class="dialogConfig" :visible.sync="openExceptionSolutiontype" width="500px" append-to-body>
            <el-form ref="formExceptionSolutiontype" :model="formExceptionSolutiontype" :rules="rulesExceptionSolutiontype" label-width="140px" :inline-message="true">
                    <el-form-item prop="categoryId" class="marginBottom0">
                      <span slot="label">
                          <span class="fontSize12"><strong>异常分类ID</strong></span>
                      </span>
                    <el-input class="inputWidth" v-model="formExceptionSolutiontype.categoryId" placeholder="请输入异常分类ID" size="mini" disabled="true"/>
                  </el-form-item>
                   <el-form-item prop="dealtypeCode" class="marginBottom0">
                      <span slot="label">
                          <span class="fontSize12"><strong>处理类型码</strong></span>
                      </span>
                      <el-input class="inputWidth" v-model="formExceptionSolutiontype.dealtypeCode" placeholder="请输入处理类型码" size="mini" />
                  </el-form-item>
                   <el-form-item prop="dealtypeDesc" class="marginBottom0">
                      <span slot="label">
                          <span class="fontSize12"><strong>处理类型描述</strong></span>
                      </span>
                      <el-input class="inputWidth" v-model="formExceptionSolutiontype.dealtypeDesc" placeholder="请输入处理类型描述" size="mini" />
                  </el-form-item>
                   <el-input class="inputWidth" v-model="formExceptionSolutiontype.status" v-show="false"/>
                   <el-form-item prop="remark" class="marginBottom0">
                      <span slot="label">
                          <span class="fontSize12"><strong>备注</strong></span>
                      </span>
                      <el-input class="inputWidth" v-model="formExceptionSolutiontype.remark" placeholder="请输入备注" size="mini" />
                  </el-form-item>
                </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="submitExceptionSolutiontype">确 定</el-button>
                <el-button @click="cancelExceptionSolutiontype">取 消</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
<script>
import { listExceptionCategorytype, getExceptionCategorytype, delExceptionCategorytype, addExceptionCategorytype, updateExceptionCategorytype } from "@/api/eng/exceptionCategorytype";
import { listExceptionReasontype, getExceptionReasontype, delExceptionReasontype, addExceptionReasontype, updateExceptionReasontype } from "@/api/eng/exceptionReasontype";
import { listExceptionSolutiontype, getExceptionSolutiontype, delExceptionSolutiontype, addExceptionSolutiontype, updateExceptionSolutiontype } from "@/api/eng/exceptionSolutiontype";
...
export default {
    ...
    methods: {
        ...
        handleExport() {
            this.download('eng/exceptionCategorytype/export', {
              ...this.queryParams
        ...
        handleAddEngExceptionReasontype() {
          this.resetExceptionReasontype();
          this.openExceptionReasontype = true;
          this.title = "新增工艺异常原因";
        },
        handleUpdateEngExceptionReasontype(row) {
          this.resetExceptionReasontype();
        ...
        handleDeleteEngExceptionReasontype(row) {
        ...
        handleExportEngExceptionReasontype() {
          this.download('eng/ExceptionReasontype/export', {
           ...this.queryParams
          }, `ExceptionReasontype_${new Date().getTime()}.xlsx`)
        },
        ...
        // 子表单重置
        resetExceptionSolutiontype() {
            this.formExceptionSolutiontype = {
                id: null,
                categoryId: this.masterId,
                dealtypeCode: null,
                dealtypeDesc: null,
            ...
            this.resetForm("formExceptionSolutiontype");
            // 按关键字段值来初始化子表单数据
            this.onExceptionSolutiontypeCategoryIdSelection(this.masterId);
        },
        handleAddEngExceptionSolutiontype() {
          this.resetExceptionSolutiontype();
          this.openExceptionSolutiontype = true;
          this.title = "新增工艺异常处理类型";
        },
        handleUpdateEngExceptionSolutiontype(row) {
          this.resetExceptionSolutiontype();
          const id = row.id || this.idsExceptionSolutiontype
          getExceptionSolutiontype(id).then(response => {
              this.formExceptionSolutiontype = response.data;
              this.openExceptionSolutiontype = true;
              this.title = "修改工艺异常处理类型";
          });
        },
        handleDeleteEngExceptionSolutiontype(row) {
          const ids = row.id || this.idsExceptionSolutiontype;
          const idz = row.id || this.captionsExceptionSolutiontype;

          this.$modal.confirm('确定删除ID为"' + idz + '"的数据项？').then(function() {
              return delExceptionSolutiontype(ids);
          }).then(() => {
            this.getExceptionSolutiontypeList();
            this.$modal.msgSuccess("删除成功");
          }).catch(() => {});
        },
        handleExportEngExceptionSolutiontype() {
          this.download('eng/ExceptionSolutiontype/export', {
           ...this.queryParams
          }, `ExceptionSolutiontype_${new Date().getTime()}.xlsx`)
        },

        /** 提交按钮 */
        submitExceptionSolutiontype() {
            this.$refs["formExceptionSolutiontype"].validate(valid => {
                if (valid) {
                  if(this.formExceptionSolutiontype.id==this.formExceptionSolutiontype.id){
                    //TODO 上述一行代码须替代或删除

                    if (this.formExceptionSolutiontype.id != null) {
                        updateExceptionSolutiontype(this.formExceptionSolutiontype)
                        .then(response => {
                            this.$modal.msgSuccess(response.msg);
                            this.openExceptionSolutiontype = false;
                            this.getExceptionSolutiontypeList();
                        })
                        .catch(error => {
                            this.$modal.msgError(error.msg);
                        ...
                        addExceptionSolutiontype(this.formExceptionSolutiontype)
                        .then(response => {
                            this.$modal.msgSuccess(response.msg);
                            this.openExceptionSolutiontype = false;
                            this.getExceptionSolutiontypeList();
