    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" v-show="showSearch" label-width="120px">
            <el-form-item prop="categoryName" class="marginBottom5">
                <span slot="label">
                    <span class="fontSize12"><strong>制程段名</strong></span>
                </span>
                <el-input
            ...
            <el-form-item prop="categoryDesc" class="marginBottom5">
                <span slot="label">
                    <span class="fontSize12"><strong>制程段描述</strong></span>
                </span>
                <el-input
                    v-model="queryParams.categoryDesc"
                    placeholder="请输入制程段描述"
                    clearable
                    size="mini"
                    class="searchInput"
                    @keyup.enter.native="handleQuery"
            ...
            <el-form-item prop="categorySeq" class="marginBottom5">
                <span slot="label">
        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="制程段名" align="left" prop="categoryName" width="160"/>
            <el-table-column label="制程段描述" align="left" prop="categoryDesc" width="240"/>
            <el-table-column label="序号" align="right" prop="categorySeq" width="60"/>
        ...
        <el-dialog :title="title" class="dialogConfig" :visible.sync="open" width="500px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                <el-form-item  prop="categoryName" class="marginBottom0">
                    <span slot="label"><span class="fontSize12"><strong>制程段名</strong></span></span>
                     <el-input class="inputWidth" v-model="form.categoryName" placeholder="请输入制程段名" size="mini" />
                </el-form-item>
                <el-form-item  prop="categoryDesc" class="marginBottom0">
                    <span slot="label"><span class="fontSize12"><strong>制程段描述</strong></span></span>
                     <el-input class="inputWidth" v-model="form.categoryDesc" placeholder="请输入制程段描述" size="mini" />
                </el-form-item>
                <el-form-item  prop="categorySeq" class="marginBottom0">
                    <span slot="label"><span class="fontSize12"><strong>序号</strong></span></span>
                     <el-input class="inputWidth" v-model="form.categorySeq" placeholder="请输入序号" size="mini" />
                </el-form-item>
...
<script>
import { listProcessCategory, getProcessCategory, delProcessCategory, addProcessCategory, updateProcessCategory } from "@/api/eng/processCategory";
...
export default {
    ...
    data() {
      return {
        ...
        rules: {
            categoryName: [
                { required: true, message: "制程段名不能为空", trigger: "blur" }
            ],
            categoryDesc: [
                { required: true, message: "制程段描述不能为空", trigger: "blur" }
            ],
            categorySeq: [
    ...
    methods: {
      ...
      rowClassName({ row, rowIndex }) {
        ...
        listProcessCategory(this.queryParams).then(response => {
          this.processCategoryList = response.rows;
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('确认删除制程段名为"' + categoryNamez + '"的数据项？').then(function() {
        ...
        handleExport() {
            this.download('eng/processCategory/export', {
             ...this.queryParams
             }, `processCategory_${new Date().getTime()}.xlsx`)
        },
