                ...
                <el-row :gutter="10" class="mb8">
                    <el-col :span="1.5">
                        <el-button
                            ...
                            :disabled="multipleMrpMr"
                            @click="handleTopr"
                        >转需求</el-button>
                    </el-col>
                </el-row>
                <el-table ref="mrpMr" border
                        :data="mrpMrList"
                        highlight-current-row
                        :row-class-name="rowClassNameMrpMr"
                        @selection-change="handleMrpMrSelectionChange"
                    ...
                    <el-table-column label="物料编码" align="left" prop="materialCode"  width="168" show-overflow-tooltip/>
                    <el-table-column label="物料名称" align="left" prop="materialName" width="140" show-overflow-tooltip/>
                    <el-table-column label="物料规格" align="left" prop="materialSpec" width="120" show-overflow-tooltip/>
...
<script>
import { listMaterialComputing, getMaterialComputing, delMaterialComputing, addMaterialComputing, updateMaterialComputing } from "@/api/pln/materialComputing";
import { runMrp } from "@/api/pln/materialComputing";
import { listMrpMr, getMrpMr, delMrpMr, addMrpMr, updateMrpMr } from "@/api/pln/mrpMr";
import { toMp } from "@/api/pln/mrpMr";
...
export default {
    name: "MaterialComputing",
    dicts: ['sales_status_mps', 'bill_priority', 'pln_mpsitem_type', 'bill_status_convertto', 'eng_material_consumingtype', 'sys_yes_no'],
    data() {
        return {
            // 遮罩层
            loading: true,
    ...
    created() {
        ...
        /** 查询外键Project数据 */
        this.getProjectList();
        /** 查询外键MaterialComputing数据 */
        this.getMaterialComputingList();
    },
    mounted: function () {
    },
    methods: {
        ...
        getMasterList() {
          this.loading = true;
            listMaterialComputing(this.queryParams).then(response => {
              this.materialComputingList = response.rows;
              this.total = response.total;
              if (this.materialComputingList && this.materialComputingList.length>0) {
        ...
        /** 查询外键MaterialComputing数据 */
        getMaterialComputingList() {
            listMaterialComputing().then(res => {
                this.fkMaterialComputingList = res.rows;
            });
        },
        /** 搜索按钮操作 */
        handleQuery() {
        ...
        /** 子表外键MpsId被选择后触发事件 **/
        onMrpMrMpsIdSelection(fkId){
            this.fkMaterialComputingList.forEach(item=>{
                if(fkId === item.id){
                   this.formMrpMr.mpsId = item.id
                }
            });
        },
        /** 子表外键UnitId被选择后触发事件 **/
        onMrpMrUnitIdSelection(fkId){
            this.fkUnitList.forEach(item=>{
                if(fkId === item.id){
                   this.formMrpMr.unitId = item.id
                }
            });
        },
        ...
        handleRunMrp(row) {
            ...
            this.$modal.confirm('确定算料 主计划号为"' + billNoz + '"的数据项吗？').then(function() {
              return runMrp(ids);
            }).then(response => {
              this.getMasterList();
        ...
        getMrpMrList() {
            ...
            listMrpMr(params).then(res => {
                this.mrpMrList = res.rows;
            });
        },
        // 多选框选中数据
        handleMrpMrSelectionChange(selection) {
            this.idsMrpMr = selection.map(item => item.id)
            this.captionsMrpMr = selection.map(item => item.materialName)
            this.singleMrpMr = selection.length!==1
            this.multipleMrpMr = !selection.length
        },
        /** 子表已算料单的CSS行样式 */
        rowClassNameMrpMr({ row, rowIndex }) {
            var clsName = ''

            if (this.idsMrpMr.includes(row.id)){
        ...
        handleTopr(row) {
            const ids = row.id || this.idsMrpMr;
            const materialNamez = row.materialName || this.captionsMrpMr;

            this.$modal.confirm('确定转需求物料名称为"' + materialNamez + '"的数据项？').then(function() {
              return toMp(ids);
            }).then(response => {
              this.getMasterList();
              this.$modal.msgSuccess(response.msg);
