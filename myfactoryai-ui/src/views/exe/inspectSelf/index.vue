<template>
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" v-show="showSearch" label-width="120px">
            ...
            <el-form-item prop="status" class="marginBottom5">
                ...
                <el-select v-model="queryParams.status" placeholder="请选择状态" clearable size="mini" class="searchInput">
                    <el-option
                        v-for="dict in dict.type.manufacturing_status_exception"
                        :key="dict.value"
                        :label="dict.label"
                        :value="dict.value"
                    />
        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="出工单" align="left" prop="wsId"  width="120">
              <template  slot-scope="scope">
                <template v-for="(item,index) in fkWorkSegmentList">
                    <span v-if="item.id==scope.row.wsId"
                          :key="index"
                          size="mini">{{item.billNo}}</span>
                </template>
              </template>
            </el-table-column>
            <el-table-column label="制造批" align="left" prop="mlId"  width="135">
              <template  slot-scope="scope">
          ...
          <el-table-column label="工序" align="center" prop="processSeq" width="50"/>
          <el-table-column label="工序码" align="center" prop="processId" width="80">
            <template  slot-scope="scope">
            ...
            <el-table-column label="产品编码" align="left" prop="materialCode"  width="168"/>
            <el-table-column label="产品名称" align="left" prop="materialName" width="168"/>
            ...
            <el-table-column label="异常数" align="right" prop="quantity" width="60"/>
            <el-table-column label="异常描述" align="left" prop="description" width="240"/>
        ...
        <el-dialog :title="title" class="dialogConfig" :visible.sync="open" width="500px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                ...
                <el-form-item prop="wsId" class="marginBottom0" >
                  <span slot="label"><span class="fontSize12"><strong>开工单</strong></span></span>
                  <el-select class="inputWidth" v-model="form.wsId" placeholder="请选择开工单" @change="onWsIdSelection"  size="mini" >
                    <el-option
                ...
                <el-form-item prop="mlId" class="marginBottom0" >
                  ...
                  <el-select class="inputWidth" v-model="form.mlId" size="mini" :disabled="true">
                    <el-option
                ...
                <el-form-item prop="processSeq" class="marginBottom0" >
                  ...
                  <el-input class="inputWidth" v-model="form.processSeq" placeholder="请选择工序号" size="mini" :disabled="true"/>
                </el-form-item>
                ...
                <el-form-item prop="inspectGroupId" class="marginBottom0">
                  ...
                  <el-select class="inputWidth" v-model="form.inspectGroupId" size="mini" :disabled="true">
                    <el-option
                ...
                <el-form-item prop="materialCode" class="marginBottom0" >
                  ...
                  <el-input class="inputWidth" v-model="form.materialCode" size="mini" :disabled="true"/>
                </el-form-item>
                ...
                <el-form-item  prop="quantityAvailable" class="marginBottom0">
                  <span slot="label"><span class="fontSize12"><strong>可异常数量</strong></span></span>
                ...
                <el-form-item  prop="quantity" class="marginBottom0">
                    <span slot="label"><span class="fontSize12"><strong>实异常数量</strong></span></span>
                ...
                <el-form-item  prop="description" class="marginBottom0">
                    <span slot="label"><span class="fontSize12"><strong>异常描述</strong></span></span>
                     <el-input class="inputWidth" type="textarea" v-model="form.description" placeholder="请输入异常描述" size="mini" />
                </el-form-item>
...
<script>
import {getNextBillno, updateBillSeq } from "@/api/exe/inspectSelf";
import { listInspectSelfByOperator, getInspectSelf, delInspectSelf, addInspectSelf, updateInspectSelf } from "@/api/exe/inspectSelf";
import { requestDeal } from "@/api/exe/inspectSelf";
...
import { listWorkSegment} from "@/api/exe/workSegment";
...
import {listUnfinishedWs} from "@/api/exe/mlOperating";

export default {
    name: "InspectSelf",
    dicts: ['manufacturing_status_exception', 'qua_exeption_dealtype'],
    data() {
      return {
        // 遮罩层
        loading: true,
        ...
        rules: {
            wsId: [
                { required: true, message: "出工单不能为空", trigger: "blur" }
            ],
            quantity: [
    ...
    methods: {
      ...
      /** 查询外键WorkSegment数据 */
      getWorkSegmentList() {
        listWorkSegment().then(res => {
          this.fkWorkSegmentList = res.rows;
        });
      },
      /** 查询外键MlTrace数据 */
      getMlTraceList() {
      ...
      /**主表外键MlId被选择后触发事件*/
      onWsIdSelection(fkId){
          this.unfinishedWorksegmentList.forEach(item=>{
              if(fkId === item.id){
                  this.form.mlId = item.mlId;
                  this.form.processSeq = item.processSeq;
                  this.form.processId = item.processId;
                  this.form.inspectGroupId = item.groupId;
                  this.form.lineId = item.lineId;
                  this.form.materialId = item.productId;
                  this.form.materialCode = item.productCode;
                  this.form.unitId=item.unitId;
                  this.form.quantityAvailable=item.quantityStart-item.quantitySplit-item.quantityScrap
                                             -item.quantityRedisp-item.quantityPending;
                  this.form.quantity=this.form.quantityAvailable;
              }
          });
      },
      ...
      onSupplierIdSelection(fkId){
        ...
        getList() {
            this.loading = true;
            listInspectSelfByOperator(this.queryParams).then(response => {
              this.inspectSelfList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getInspectSelf(id).then(response => {
        ...
        handleExport() {
            this.download('exe/inspectSelf/export', {
             ...this.queryParams
             }, `inspectSelf_${new Date().getTime()}.xlsx`)
        },
        handleRequestDeal(row) {
            this.$modal.confirm('确定请判 异常单号为"' + row.billNo + '"的数据项吗？').then(function() {
                 return requestDeal(row);
            }).then(response => {
                this.getList();
      ...
      submit() {
        this.$refs["form"].validate(valid => {
          if (valid) {
            if(this.form.quantity>0 && this.form.quantity<=this.form.quantityAvailable){
                if (this.form.id != null) {
                  updateInspectSelf(this.form)
                  .then(response => {
                    this.$modal.msgSuccess(response.msg);
                    this.open = false;
                    this.getList();
              ...
              this.$modal.msgWarning("实异常数量应大于0且小于等于可异常数量！");
            }
          }
        });
      },
