...
<script>
import {getNextBillno, updateBillSeq } from "@/api/exe/inspectInprocess";
import { listInspectInprocess, getInspectInprocess, delInspectInprocess, addInspectInprocess, updateInspectInprocess } from "@/api/exe/inspectInprocess";
import { requestDeal } from "@/api/exe/inspectInprocess";
...
/**主表外键DealBy被选择后触发事件*/
        ...
        getList() {
            this.loading = true;
        listInspectInprocess(this.queryParams).then(response => {
          this.inspectInprocessList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getInspectInprocess(id).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('确认删除异常单号为"' + billNoz + '"的数据项？').then(function() {
              return delInspectInprocess(ids);
            }).then(() => {
              this.getList();
        ...
        handleExport() {
            this.download('exe/inspectInprocess/export', {
             ...this.queryParams
             }, `inspectInprocess_${new Date().getTime()}.xlsx`)
        },
            ...
            if(this.form.id==this.form.id){
              ...
              //TODO 上述两行代码须替代或删除
                ...
                if (this.form.id != null) {
                  updateInspectInprocess(this.form)
                  .then(response => {
                    this.$modal.msgSuccess(response.msg);
                    this.open = false;
                    this.getList();
