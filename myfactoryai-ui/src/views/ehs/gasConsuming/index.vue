        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="设备" align="right" prop="foodieId"  width="120">
                <template  slot-scope="scope">
                    <template v-for="(item,index) in fkGasFoodieList">
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                <el-form-item prop="foodieId" class="marginBottom0" >
                    ...
                    <el-select class="inputWidth" v-model="form.foodieId" placeholder="请选择设备"
                      ...
                      <el-option
                          v-for="(fkrow, index) in fkGasFoodieList"
                          :key="index"
                          :label="fkrow.name"
                          :value="fkrow.id"
                          size="mini"
...
<script>
import { listGasConsuming, getGasConsuming, delGasConsuming, addGasConsuming, updateGasConsuming } from "@/api/ehs/gasConsuming";
import { listUnit} from "@/api/bas/unit";
import { listGasFoodie} from "@/api/ehs/gasFoodie";
    ...
    methods: {
      ...
      getUnitList() {
        listUnit().then(res => {
          ...
          /** 查询外键GasFoodie数据 */
      getGasFoodieList() {
        listGasFoodie().then(res => {
          this.fkGasFoodieList = res.rows;
        });
      },

/**主表外键FoodieId被选择后触发事件*/
        onFoodieIdSelection(fkId){
            this.fkGasFoodieList.forEach(item=>{
                if(fkId === item.id){
...
/**主表外键UnitId被选择后触发事件*/
        ...
        getList() {
            this.loading = true;
        listGasConsuming(this.queryParams).then(response => {
          this.gasConsumingList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getGasConsuming(id).then(response => {
        ...
        handleExport() {
            this.download('ehs/gasConsuming/export', {
             ...this.queryParams
             }, `gasConsuming_${new Date().getTime()}.xlsx`)
        },
