        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="设备" align="right" prop="foodieId"  width="120">
                <template  slot-scope="scope">
                    <template v-for="(item,index) in fkWaterFoodieList">
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                <el-form-item prop="foodieId" class="marginBottom0" >
                    ...
                    <el-select class="inputWidth" v-model="form.foodieId" placeholder="请选择设备"
                      ...
                      <el-option
                          v-for="(fkrow, index) in fkWaterFoodieList"
                          :key="index"
                          :label="fkrow.name"
                          :value="fkrow.id"
                          size="mini"
...
<script>
import { listWaterConsuming, getWaterConsuming, delWaterConsuming, addWaterConsuming, updateWaterConsuming } from "@/api/ehs/waterConsuming";
import { listUnit} from "@/api/bas/unit";
import { listWaterFoodie} from "@/api/ehs/waterFoodie";
    ...
    methods: {
      ...
      getUnitList() {
        listUnit().then(res => {
          ...
          /** 查询外键WaterFoodie数据 */
      getWaterFoodieList() {
        listWaterFoodie().then(res => {
          this.fkWaterFoodieList = res.rows;
        });
      },

/**主表外键FoodieId被选择后触发事件*/
        onFoodieIdSelection(fkId){
            this.fkWaterFoodieList.forEach(item=>{
                if(fkId === item.id){
...
/**主表外键UnitId被选择后触发事件*/
        ...
        getList() {
            this.loading = true;
        listWaterConsuming(this.queryParams).then(response => {
          this.waterConsumingList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getWaterConsuming(id).then(response => {
        ...
        handleExport() {
            this.download('ehs/waterConsuming/export', {
             ...this.queryParams
             }, `waterConsuming_${new Date().getTime()}.xlsx`)
        },
            ...
            if(this.form.id==this.form.id){
              ...
              //TODO 上述两行代码须替代或删除
                ...
                if (this.form.id != null) {
                  updateWaterConsuming(this.form)
                  .then(response => {
