 ...
 :disabled="multiple"                     @click="handleDelete"
        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="设备分类" align="right" prop="categoryId"  width="120">
                <template  slot-scope="scope">
                    <template v-for="(item,index) in fkGasFoodieCategoryList">
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                ...
                <el-form-item prop="categoryId" class="marginBottom0" >
                    ...
                    <el-select class="inputWidth" v-model="form.categoryId" placeholder="请选择设备分类"
                      ...
                      <el-option
                          v-for="(fkrow, index) in fkGasFoodieCategoryList"
                          :key="index"
                          :label="fkrow.name"
                          :value="fkrow.id"
                          size="mini"
...
<script>
import { listGasFoodie, getGasFoodie, delGasFoodie, addGasFoodie, updateGasFoodie } from "@/api/ehs/gasFoodie";
import { listGasFoodieCategory} from "@/api/ehs/gasFoodieCategory";
...
export default {
    ...
    created() {
      this.getList();
          /** 查询外键GasFoodieCategory数据 */
          this.getGasFoodieCategoryList();
    },
    methods: {
      /** 表格行样式 */
      rowClassName({ row, rowIndex }) {
        ...
        return className;
          ...
          /** 查询外键GasFoodieCategory数据 */
      getGasFoodieCategoryList() {
        listGasFoodieCategory().then(res => {
          this.fkGasFoodieCategoryList = res.rows;
        });
      },

/**主表外键CategoryId被选择后触发事件*/
        onCategoryIdSelection(fkId){
            this.fkGasFoodieCategoryList.forEach(item=>{
                if(fkId === item.id){
        ...
        getList() {
            this.loading = true;
        listGasFoodie(this.queryParams).then(response => {
          this.gasFoodieList = response.rows;
        ...
        handleExport() {
            this.download('ehs/gasFoodie/export', {
             ...this.queryParams
             }, `gasFoodie_${new Date().getTime()}.xlsx`)
        },
