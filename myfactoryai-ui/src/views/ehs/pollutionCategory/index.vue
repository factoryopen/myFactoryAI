...
<script>
import { listPollutionCategory, getPollutionCategory, delPollutionCategory, addPollutionCategory, updatePollutionCategory } from "@/api/ehs/pollutionCategory";
...
export default {
    ...
    methods: {
      ...
      rowClassName({ row, rowIndex }) {
        ...
        getList() {
            this.loading = true;
        listPollutionCategory(this.queryParams).then(response => {
          this.pollutionCategoryList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getPollutionCategory(id).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('确认删除名称为"' + namez + '"的数据项？').then(function() {
              return delPollutionCategory(ids);
            }).then(() => {
              this.getList();
        ...
        handleExport() {
            this.download('ehs/pollutionCategory/export', {
             ...this.queryParams
             }, `pollutionCategory_${new Date().getTime()}.xlsx`)
        },
            ...
            if(this.form.id==this.form.id){
              ...
              //TODO 上述两行代码须替代或删除
                ...
                if (this.form.id != null) {
                  updatePollutionCategory(this.form)
                  .then(response => {
                  ...
                  addPollutionCategory(this.form)
                  .then(response => {
                    this.$modal.msgSuccess(response.msg);
                    this.open = false;
                    this.getList();
