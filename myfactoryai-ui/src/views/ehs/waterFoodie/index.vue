 ...
 :disabled="multiple"                     @click="handleDelete"
        ...
        <el-table v-loading="loading" border
            ...
            <el-table-column label="设备分类" align="right" prop="categoryId"  width="120">
                <template  slot-scope="scope">
                    <template v-for="(item,index) in fkWaterFoodieCategoryList">
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline-message="true">
                ...
                <el-form-item prop="categoryId" class="marginBottom0" >
                    ...
                    <el-select class="inputWidth" v-model="form.categoryId" placeholder="请选择设备分类"
                      ...
                      <el-option
                          v-for="(fkrow, index) in fkWaterFoodieCategoryList"
                          :key="index"
                          :label="fkrow.name"
                          :value="fkrow.id"
                          size="mini"
...
<script>
import { listWaterFoodie, getWaterFoodie, delWaterFoodie, addWaterFoodie, updateWaterFoodie } from "@/api/ehs/waterFoodie";
import { listWaterFoodieCategory} from "@/api/ehs/waterFoodieCategory";
...
export default {
    ...
    created() {
      this.getList();
          /** 查询外键WaterFoodieCategory数据 */
          this.getWaterFoodieCategoryList();
    },
    methods: {
      /** 表格行样式 */
      rowClassName({ row, rowIndex }) {
        ...
        return className;
          ...
          /** 查询外键WaterFoodieCategory数据 */
      getWaterFoodieCategoryList() {
        listWaterFoodieCategory().then(res => {
          this.fkWaterFoodieCategoryList = res.rows;
        });
      },

/**主表外键CategoryId被选择后触发事件*/
        onCategoryIdSelection(fkId){
            this.fkWaterFoodieCategoryList.forEach(item=>{
                if(fkId === item.id){
        ...
        getList() {
            this.loading = true;
        listWaterFoodie(this.queryParams).then(response => {
          this.waterFoodieList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getWaterFoodie(id).then(response => {
        ...
        handleExport() {
            this.download('ehs/waterFoodie/export', {
             ...this.queryParams
             }, `waterFoodie_${new Date().getTime()}.xlsx`)
        },
