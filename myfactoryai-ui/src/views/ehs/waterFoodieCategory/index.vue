...
<script>
import { listWaterFoodieCategory, getWaterFoodieCategory, delWaterFoodieCategory, addWaterFoodieCategory, updateWaterFoodieCategory } from "@/api/ehs/waterFoodieCategory";

export default {
    name: "WaterFoodieCategory",
    ...
    methods: {
      ...
      rowClassName({ row, rowIndex }) {
        ...
        getList() {
            this.loading = true;
        listWaterFoodieCategory(this.queryParams).then(response => {
          this.waterFoodieCategoryList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getWaterFoodieCategory(id).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('确认删除名称为"' + namez + '"的数据项？').then(function() {
              return delWaterFoodieCategory(ids);
            }).then(() => {
              this.getList();
        ...
        handleExport() {
            this.download('ehs/waterFoodieCategory/export', {
             ...this.queryParams
             }, `waterFoodieCategory_${new Date().getTime()}.xlsx`)
        },
            ...
            if(this.form.id==this.form.id){
              ...
              //TODO 上述两行代码须替代或删除
                ...
                if (this.form.id != null) {
                  updateWaterFoodieCategory(this.form)
                  .then(response => {
                  ...
                  addWaterFoodieCategory(this.form)
                  .then(response => {
                    this.$modal.msgSuccess(response.msg);
                    this.open = false;
                    this.getList();
