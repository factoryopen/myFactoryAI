...
<script>
import { listPowerFoodieCategory, getPowerFoodieCategory, delPowerFoodieCategory, addPowerFoodieCategory, updatePowerFoodieCategory } from "@/api/ehs/powerFoodieCategory";

export default {
    name: "PowerFoodieCategory",
    ...
    methods: {
      ...
      rowClassName({ row, rowIndex }) {
        ...
        getList() {
            this.loading = true;
        listPowerFoodieCategory(this.queryParams).then(response => {
          this.powerFoodieCategoryList = response.rows;
        ...
        handleUpdate(row) {
            ...
            getPowerFoodieCategory(id).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('确认删除名称为"' + namez + '"的数据项？').then(function() {
              return delPowerFoodieCategory(ids);
            }).then(() => {
              this.getList();
        ...
        handleExport() {
            this.download('ehs/powerFoodieCategory/export', {
             ...this.queryParams
             }, `powerFoodieCategory_${new Date().getTime()}.xlsx`)
        },
            ...
            if(this.form.id==this.form.id){
              ...
              //TODO 上述两行代码须替代或删除
                ...
                if (this.form.id != null) {
                  updatePowerFoodieCategory(this.form)
                  .then(response => {
                  ...
                  addPowerFoodieCategory(this.form)
                  .then(response => {
                    this.$modal.msgSuccess(response.msg);
                    this.open = false;
                    this.getList();
