                ...
                <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width" width="71">
                  <template slot-scope="scope">
                      <el-button
                           ...
                           @click="handleAcceptInspection(scope.row)"
                      >收检</el-button>
                  </template>
              </el-table-column>
        </el-table>
        ...
        <el-tabs>
            <el-tab-pane label="请检明细">
                ...
                <el-table ref="inspectRequestItem" border
                    ...
                    <el-table-column label="物料" align="right" prop="materialCode"  width="120"/>
                    ...
                    <el-table-column label="图号" align="left" prop="drawNo" width="110"/>
                    ...
                    <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width" width="180">
                       <template slot-scope="scope">
                           <el-button
                             ...
                             @click="handleRegisterGoodInspection(scope.row)"
                           >合格</el-button>
                           <el-button
                               size="mini"
                               type="text"
                               icon="el-icon-search"
                               :disabled="scope.row.status!=5"
                               @click="handleRegisterExceptionalInspection(scope.row)"
                           >检异</el-button>
                           <el-button
                               size="mini"
                               type="text"
                               ...
                               :disabled="scope.row.status!=5"
                               @click="handleRequestJugde(scope.row)"
                           >请判</el-button>
                       </template>
                    </el-table-column>
                </el-table>
...
export default {
    ...
    methods: {
        ...
        handleRegisterGoodInspection(row) {
          ...
          const inpectionItem = row;
          const _that=this;
          getInspectRequestItem(id).then(response => {
            this.$modal.confirm('确定项次为 ['+ this.masterNo +","+ row.seqNo + ']的检验结果为 合格？').then(function() {
              registerGoodInspection(requestId, inpectionItem).then(response => {
                _that.getMasterList();
                _that.$modal.msgSuccess(response.msg);
                _that.openInspectRequestItemregisterInspection = false;
              });
            });
          });
        },
